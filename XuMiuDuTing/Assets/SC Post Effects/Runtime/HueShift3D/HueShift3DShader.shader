Shader "Hidden/SC Post Effects/3D Hue Shift"
{
	...
	float4 FragSpectrum(Varyings input): SV_Target
	{
		...
		half4 normals = SAMPLE_DEPTH_NORMALS(SCREEN_COORDS);

		float3 noiseCoords = float3(SCREEN_COORDS.x * _Params.y, SCREEN_COORDS.y * _Params.y, (_Time.y * _Params.x));

		half noise = 0.5 + 0.5 * snoise(noiseCoords + ( normals.rgb * _Params.z));
		half waveSize = 0.5;

		//Add waves
		noise = noise + RadialWave(SCREEN_COORDS * waveSize, float2(0.5, 0.5) * waveSize);
	...
	float4 FragGradientTex(Varyings input) : SV_Target
	{
		...
		float3 hsv = float3(noise, 1, 1);
		//RGB
		float4 col = SAMPLE_TEXTURE2D(_GradientTex, sampler_LinearRepeat, float2(noise, 0));
