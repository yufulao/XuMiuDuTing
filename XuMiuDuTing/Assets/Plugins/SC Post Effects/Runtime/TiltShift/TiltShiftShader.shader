Shader "Hidden/SC Post Effects/Tilt Shift"
{
	...
	float4 FragHorizontal(Varyings input) : SV_Target
	{
		...
		float mask = HorizontalMask(SCREEN_COORDS.xy) * 0.01;

		float4 poissonScale = mask * _Params.z;

		for (int l = 0; l < SmallDiscKernelSamples; l++)
		{
			float2 sampleUV = SCREEN_COORDS.xy + SmallDiscKernel[l].xy * poissonScale.xy;
	...
	float4 FragHorizontalHQ(Varyings input) : SV_Target
	{
		...
		{
			float2 sampleUV = SCREEN_COORDS.xy + DiscKernel[l].xy * poissonScale.xy;
			float3 sample0 = ScreenColor(sampleUV.xy).rgb;
	...
	float4 FragRadial(Varyings input) : SV_Target
	{
		...
		float mask = RadialMask(SCREEN_COORDS.xy) * 0.01;
	...
	{
		UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
		float mask = _Params.w == 0 ? HorizontalMask(SCREEN_COORDS.xy) : RadialMask(SCREEN_COORDS.xy);
